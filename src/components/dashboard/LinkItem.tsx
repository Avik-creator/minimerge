...
const LinkItem: React.FC<LinkItemType> = ({
  ...
  const deleteLink = async () => {
    ...
    if (ok) {
      try {
        const res = await axios.delete("/api/link/delete-link", {
          headers: { linkId: id },
        });
  ...
  const updateLink = async (e: any) => {
    ...
    try {
      const res = await axios.patch("/api/link/update-link", {
        linkId: id,
        destinationUrl,
        password: pswd,
  ...
  return (
    <div className="w-[100%] h-[100%] flex items-center justify-between px-3 bg-white boxshadow-two">
      {
        <div
          className={`absolute w-[100%] h-[calc(100vh-7rem)] left-1/2 top-1/2 py-5 bg-white z-50 transform -translate-x-1/2 -translate-y-1/2 ${
            openQR ? "block" : "hidden"
        ...
        >
          ...
          <canvas
            className="w-[70%] md:w-[20%] mt-10 mb-5"
            id="canvas"
            ref={canvasRef}
          />
          <h1>{shortUrl}</h1>
        ...
        <div className="flex flex-col items-start justify-between py-3">
          <div className="flex items-center justify-center gap-3">
            <a href={`${shortUrl}`} target="_blank">
              <h1 className="text-[#1e40b4] font-bold">
                {shortUrl.substring(8, shortUrl.length)}
              </h1>
            </a>
        ...
        <div>
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              ...
              <DropdownMenuItem onClick={duplicateLink}>
                Duplicate
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => {
