...
const CreateLinkModal = ({ getLinks }: CreateModalPropsType) => {
    ...
    try {
      ...
      if (!res.data.error) {
        getLinks().then((links) => {
          setLinksState({
            loading: false,
            links: links == undefined ? [] : sortLinks(links, filter),
          });
        });
        modalTriggerRef.current?.click();
  ...
  return (
    <Dialog>
      <DialogTrigger>
        <Button ref={modalTriggerRef} style={{ borderRadius: ".6rem" }}>
          <div className="flex justify-center items-center gap-2">
            <span>Create Link</span>
            <div className="px-2 py-1 bg-[#3f3f46] text-[#6d7584] rounded-xl">
              C
            </div>
          </div>
        </Button>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Create a new Minimerge</DialogTitle>
        </DialogHeader>
