...
const Settings = () => {
  ...
  const [errors, setErros] = useState({
    username: { error: false, msg: "", disabled: true },
    avatar: { error: false, msg: "", disabled: true },
  });
  ...
  const updateAccount = async (option: string) => {
    if (option === "username") {
      try {
        ...
        setUser(updatedUser);
        setErros({
          username: { ...errors.username, disabled: true },
          avatar: { ...errors.avatar },
        });
      } catch (e) {
        toast({ description: "Something went wrong!", variant: "destructive" });
      ...
      try {
        if (file) {
          ...
          setErros({
            ...
            avatar: { ...errors.avatar, disabled: true },
          });
        }
      } catch (e) {
        toast({ description: "Something went wrong!", variant: "destructive" });
    ...
    <div className="w-[100vw] h-[calc(100vh-8rem)] overflow-x-hidden flex flex-col items-center justify-start">
      ...
      <div className="w-[100%] flex items-center justify-center">
        <div className="md:w-[60%] w-[88%] flex px-5 py-3 justify-between items-center mt-1">
          <div className="w-[100%] flex justify-start items-center gap-4"></div>
        </div>
      </div>
      <div className="md:w-[90%] w-[88%] h-[5.5rem]">
        <div className="w-[100%] flex items-center justify-center py-[2rem]">
          <div className="flex flex-col gap-5 md:w-[60%] w-[88%]">
            <div
              className="w-[100%] flex flex-col gap-5 items-start h-fit px-[3rem] py-[2rem]"
              id="boxshadow-two"
            >
              <h1 className="font-medium text-2xl">Username</h1>
              {errors.username.error && (
                <Label style={{ color: "red" }}>{errors.username.msg}</Label>
              )}
              <div className="w-[100%] flex items-center justify-between gap-2">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setErros({
                      username: { ...errors.username, disabled: false },
                      avatar: { ...errors.avatar },
                    });
                  }}
                />
                <Button
                  disabled={errors.username.disabled && username !== ""}
            ...
            >
              ...
              <div className="w-[100%] flex items-center justify-between gap-2">
                {errors.avatar.error && (
                  <Label style={{ color: "red" }}>{errors.avatar.msg}</Label>
                )}
                <input
                  type="file"
                  ...
                  onChange={(e) => {
                    setErros({
                      username: { ...errors.username },
                      avatar: { ...errors.avatar, disabled: false },
                    });
                    setFile(e.target.files![0]);
                  }}
                />
                <Button
                  disabled={errors.avatar.disabled}
                  onClick={() => updateAccount("avatar")}
                >
                  Save
                </Button>
              </div>
            ...
            >
              ...
              <span>
                Permanently delete your Minimerge account and all of your links
                + their stats. This action cannot be undone - please proceed
                with caution.
              </span>
