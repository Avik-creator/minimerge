...
const Dashboard = () => {
  ...
  const getLinks = async (): Promise<ILink[] | undefined> => {
    try {
      const res = await axios.post("/api/link/get-links", {
  ...
  useEffect(() => {
    getLinks().then((d) => {
      setLinksState({
        loading: false,
        links: d == undefined ? [] : sortLinks(d, filter),
      });
    });
  }, [filter]);
  ...
  if (status == "authenticated" && !flag) {
    ...
    getLinks().then((d) => {
      setLinksState({
        loading: false,
        links: d == undefined ? [] : sortLinks(d, "clicks"),
      });
    });
  }
        ...
        <div className="w-[100vw] h-[100%] overflow-x-hidden flex flex-col items-center justify-start">
          ...
          <div className="md:w-[60%] w-[88%] h-[5.5rem]">
            {linksState.links.length == 0 && (
              <h1 className="text-center">Create your first Minimerge!</h1>
            )}
                ...
                {linksState.links.map((link: ILink) => (
                  <div className="mt-4 w-[100%]">
                    <LinkItem
                      ...
                      isProtected={link.isPrivate}
                      password={link.password}
                      id={link.id}
                      getLinks={getLinks}
                      favicon={`http://www.google.com/s2/favicons?domain=${link.url}`}
                      clicks={link.clicks}
                      createdAt={"20m"}
